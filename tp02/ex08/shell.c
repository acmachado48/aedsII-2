...
void ordenarPokemons(Pokedex *pokedex)
{
    ...
    for (int gap = n / 2; gap > 0; gap /= 2)
    {
        for (int i = gap; i < n; i++)
        {
            Pokemon temp = pokedex->listaDePokemons[i];
            int j;
            for (j = i; j >= gap && ((temp.weight < pokedex->listaDePokemons[j - gap].weight) ||
                                     (temp.weight == pokedex->listaDePokemons[j - gap].weight &&
                                      strcmp(temp.name, pokedex->listaDePokemons[j - gap].name) < 0));
                 j -= gap)
            {
                pokedex->listaDePokemons[j] = pokedex->listaDePokemons[j - gap];
              
            }
            pokedex->listaDePokemons[j] = temp;
