...
void quicksort(Pokemon *lista, int low, int high)
{
    if (low < high)
    {
...
int partition(Pokemon *lista, int low, int high)
{
    ...
    {
        if (lista[j].height < pivot.height ||
            (lista[j].height == pivot.height && strcmp(lista[j].name, pivot.name) < 0))
        {
            i++;
...
{
    quicksort(pokedex->listaDePokemons, 0, pokedex->numPokemons - 1);
}
